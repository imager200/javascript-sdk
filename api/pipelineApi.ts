...
import { PipelineExecutionResult } from '../model/pipelineExecutionResult';
...
export enum PipelineApiApiKeys {
    ApiKeyAuth,
}
...
export class PipelineApi {
    ...
    public setApiKey(key: PipelineApiApiKeys, value: string) {
        (this.authentications as any)[PipelineApiApiKeys[key]].apiKey = value;
    }
     ...
     */
    public async pipelineGet (url: string, name: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: SuccessResponse;  }> {
        const localVarPath = this.basePath + '/pipeline/{name}'
        ...
        if (url === null || url === undefined) {
            throw new Error('Required parameter url was null or undefined when calling pipelineGet.');
        }

        // verify required parameter 'name' is not null or undefined
        if (name === null || name === undefined) {
            throw new Error('Required parameter name was null or undefined when calling pipelineGet.');
        }
     ...
     * @param executionId the execution id obtained from the pipeline creation response
     */
    public async pipelineGetStatus (executionId: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PipelineExecutionResult;  }> {
        const localVarPath = this.basePath + '/pipeline/{execution_id}/status'
        ...
        if (executionId === null || executionId === undefined) {
            throw new Error('Required parameter executionId was null or undefined when calling pipelineGetStatus.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: PipelineExecutionResult;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "PipelineExecutionResult");
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
                        }
     ...
     * 
     * @param name the name of the pipeline
     * @param body image binary data, acceptable formats: jpeg,png,bmp,gif (depending on operations)
     */
    public async pipelinePost (name: string, body: RequestFile, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: SuccessResponse;  }> {
        const localVarPath = this.basePath + '/pipeline/{name}'
            .replace('{' + 'name' + '}', encodeURIComponent(String(name)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        ...
        // verify required parameter 'name' is not null or undefined
        if (name === null || name === undefined) {
            throw new Error('Required parameter name was null or undefined when calling pipelinePost.');
        }

        // verify required parameter 'body' is not null or undefined
        if (body === null || body === undefined) {
            throw new Error('Required parameter body was null or undefined when calling pipelinePost.');
        }
