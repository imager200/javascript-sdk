...
export enum ImageinfoApiApiKeys {
    ApiKeyAuth,
}

export class ImageinfoApi {
    ...
    public setApiKey(key: ImageinfoApiApiKeys, value: string) {
        (this.authentications as any)[ImageinfoApiApiKeys[key]].apiKey = value;
    }
     ...
     */
    public async imageinfoSyncGet (url: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ImageMetadata;  }> {
        const localVarPath = this.basePath + '/imageinfo';
        ...
        if (url === null || url === undefined) {
            throw new Error('Required parameter url was null or undefined when calling imageinfoSyncGet.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: ImageMetadata;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "ImageMetadata");
     ...
     */
    public async imageinfoSyncPost (body: RequestFile, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ImageMetadata;  }> {
        const localVarPath = this.basePath + '/imageinfo';
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        const produces = ['application/json'];
        ...
        if (body === null || body === undefined) {
            throw new Error('Required parameter body was null or undefined when calling imageinfoSyncPost.');
        }
